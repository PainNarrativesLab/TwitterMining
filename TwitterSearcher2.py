		...
		try:
			...
			for _ in range(self.MAX_PAGES-1):  # Get more pages
					if self.recent == 'False': #search for older tweets					
						search_results = self.twitter_conn.search.tweets(q=hashtag, count=100, max_id=self.limitTweet)  #tweets before ealiest record
						if len(search_results) > 0:
							self.process_search_results(search_results['statuses'])
					elif self.recent == 'True': #search for newer tweets
						search_results = self.twitter_conn.search.tweets(q=hashtag, count=100, since_id=self.limitTweet)  #get most recent tweets
						if len(search_results) > 0:
