                                ...
                                for _ in range(self.MAX_PAGES-1):  # Get more pages
                                          try:
                                                    if recent == 'False':
                                                                print 'doing old'
                                                                self.get_oldest_id()
                                                                self.search_results = self.twitter_conn.search.tweets(q=self.query, count=100, max_id=self.oldest)  #tweets before ealiest record
                                                    else:
                                                                print 'doing new'
                                                                #self.get_newest_id()
                                                                self.redis_service.get_max_id()
                                                                print 'Newest stored tweet %s' % self.redis_service.maxid
                                                                self.search_results = self.twitter_conn.search.tweets(q=self.query, count=100, since_id=self.redis_service.maxid)  #get most recent tweets
